...
open class NetStream: NSObject {
...
#if os(iOS) || os(macOS)
    open func attachCamera(_ camera:AVCaptureDevice?, onError:((_ error:NSError) -> Void)? = nil, onCompletion:(() -> Void)? = nil) {
        lockQueue.async {
            do {
    ...
    open func attachAudio(_ audio:AVCaptureDevice?, automaticallyConfiguresApplicationAudioSession:Bool = false, onError:((_ error:NSError) -> Void)? = nil, onCompletion:(() -> Void)? = nil) {
        lockQueue.async {
            do {
